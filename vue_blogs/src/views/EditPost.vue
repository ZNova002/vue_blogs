<template>
  ...
  <section class="post-section">
    <div class="post-container">
      <h1 class="post-title">Chỉnh sửa bài viết</h1>
      ...
      <div v-if="thumbnailBlobUrl || post?.thumbnail" class="image-preview">
        <img
          :src="thumbnailBlobUrl || post?.thumbnail"
      ...
      <div v-if="imageBlobUrls.length || post?.images?.length" class="image-preview">
        <img
          v-for="(preview, idx) in imageBlobUrls"
          :key="'new-' + idx"
          :src="preview"
          alt="Image Preview"
      ...
      <button @click="updatePostHandler" class="submit-btn" :disabled="isLoading">Cập nhật bài viết</button>
    </div>
  </section>
  <Footer />
</template>
...
import { getPost, updatePost, getCategories, getTags } from '@/services/createPost';
...
onMounted(async () => {
  try {
    ...
    content.value = post.value.content || '<p></p>';
    selectedCategory.value = post.value.category_id;

    // Lấy danh sách tags của bài viết
    const tagsResponse = await getTagsByPostId(route.params.id);
    if (tagsResponse.success) {
      tags.value = tagsResponse.data.map(tag => tag.name);
    } else {
      tags.value = [];
      console.warn('Không lấy được tags:', tagsResponse.message);
    }
    ...
    errorMessage.value = error.message || 'Lỗi khi tải dữ liệu bài viết';
...
const updatePostHandler = async () => {
  ...
  formData.append('_method', 'PUT'); // Laravel yêu cầu _method cho PUT
  if (thumbnail.value) {
  ...
  try {
    const result = await updatePost(route.params.id, formData);
    if (result.success) {
      successMessage.value = 'Cập nhật bài viết thành công!';
      setTimeout(() => {
        router.push({ name: 'manage-posts' });
      }, 2000);
    } else {
      errorMessage.value = result.message;
    }
  } catch (error) {
    errorMessage.value = error.message || 'Lỗi khi cập nhật bài viết';
  } finally {
    isLoading.value = false;
  }
};
